...
def test_cookie_pydantic() -> None:
    parsed = RyCookieModel(cookie="name=value; HttpOnly").cookie  # type: ignore[arg-type]  # ty:ignore[invalid-argument-type]
    assert parsed == ry.Cookie.parse("name=value; HttpOnly")
    assert parsed.http_only is True

    parsed_bytes = RyCookieModel(cookie=b"name=value; Secure").cookie  # type: ignore[arg-type]  # ty:ignore[invalid-argument-type]
...
def test_cookie_pydantic_fails() -> None:
    with pytest.raises(pydantic.ValidationError, match="Cookie validation error"):
        RyCookieModel(cookie="=value")  # type: ignore[arg-type]  # ty:ignore[invalid-argument-type]
