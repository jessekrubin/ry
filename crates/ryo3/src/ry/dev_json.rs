...
pub fn stringify_v5<'py>(py: Python<'py>, obj: Bound<'py, PyAny>) -> PyResult<ryo3_bytes::PyBytes> {
    ...
    let s = serde_json::to_vec(&s)
        .map_err(|e| PyOSError::new_err(format!("Failed to serialize: {e}")))?;
...
#[pyfunction]
pub fn stringify_v6<'py>(py: Python<'py>, obj: Bound<'py, PyAny>) -> PyResult<ryo3_bytes::PyBytes> {
    // static ORJSON: GILOnceCell<Py<PyAny>> = GILOnceCell::new();
    // let a = ORJSON.import(py, "orjson", "dumps")?;
    //
    // let r = a.call1((obj,));
...
pub fn pymod_add(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(stringify_v6, m)?)?;
    Ok(())
}
