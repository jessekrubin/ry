...
use crate::response_data::RyResponseData;
...
impl RyResponse {
    ...
    #[must_use] pub fn new(res: reqwest::Response) -> Self {
        Self {
            data: RyResponseData::from(&res),
...
impl RyResponse {
    ...
    fn __str__(&self) -> String {
        format!("Response<{}>", self.data.status_code)
    }
    ...
    #[getter]
    fn status(&self) -> u16 {
        self.data.status_code.as_u16()
    }

    #[getter]
    fn status_text<'py>(&self, py: Python<'py>) -> Option<&Bound<'py, PyString>> {
        status_code_pystring(py, self.data.status_code.as_u16())
    }

    #[getter]
    fn status_code(&self) -> PyHttpStatus {
        PyHttpStatus(self.data.status_code)
    }

    /// Returns true if the response was redirected
    #[getter]
    fn redirected(&self) -> bool {
        self.data.status_code.is_redirection()
    }

    #[getter]
    fn version(&self) -> HttpVersion {
        HttpVersion(self.data.version)
    }
    ...
    fn url(&self) -> Option<PyUrl> {
        self.data.url.as_ref().map(|url| PyUrl(url.clone()))
    }

    #[getter]
    fn headers(&self) -> PyHeaders {
        let c = self.data.headers.clone();
        PyHeaders::from(c)
    }
    ...
    #[getter]
    fn ok(&self) -> bool {
        self.data.status_code.is_success()
    }
    ...
    fn content_encoding(&self) -> Option<String> {
        self.data.headers.get(CONTENT_ENCODING).map(|en| {
