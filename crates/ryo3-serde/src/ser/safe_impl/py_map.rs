...
impl Serialize for SerializePyDict<'_, '_> {
        ...
        S: Serializer,
    {
        ...
        for (k, element) in py_dict.iter() {
            let sk = SerializePyMappingKey::new(self.ctx, k.as_borrowed());
            // let sv = SerializePyAny::new_with_depth(&v, self.ctx, self.depth + 1);
            let ob_type = self.ctx.typeref.obtype(&element);
            m.serialize_key(&sk)?;
            serialize_map_value!(ob_type, m, self, element.as_borrowed());
        }
        m.end()
    }
}
...
impl Serialize for SerializePyMapping<'_, '_> {
        ...
        S: Serializer,
    {
        ...
        for (k, v) in keys.iter().zip(values.iter()) {
            ...
            m.serialize_key(&sk)?;
            serialize_map_value!(ob_type, m, self, v.as_borrowed());
        }
