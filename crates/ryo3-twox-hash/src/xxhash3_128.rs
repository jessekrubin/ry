...
impl PyXxHash3_128 {
    ...
    fn __repr__(&self) -> PyResult<PyAsciiString> {
        self.hasher
            .py_lock()
            .map(|hasher| format!("xxh3_128<{:x}>", hasher.finish_128()).into())
    }
    ...
    fn oneshot_int(
        ...
        let slice = data.as_ref();
        if slice.len() > HASHLIB_GIL_MINSIZE {
            py.detach(|| oneshot_impl(slice, seed, secret))
        } else {
            oneshot_impl(slice, seed, secret)
        }
    }
...
pub fn xxh3_128_intdigest(
    ...
    PyXxHash3_128::oneshot_int(py, data, seed, secret)
}
...
pub fn xxh3_128_hexdigest(
    ...
    PyXxHash3_128::oneshot_hex(py, data, seed, secret)
}
