...
use crate::{
    JiffDateTime, JiffEra, JiffEraYear, JiffRoundMode, JiffTzDisambiguation, JiffTzOffsetConflict,
    ...
    fn try_from<'py>(
        ...
        if let Ok(pystr) = value.downcast::<pyo3::types::PyString>() {
            ...
            Err(pyo3::exceptions::PyNotImplementedError::new_err(
                "ZonedDateTime from float not implemented",
            ))
        } else if let Ok(v) = value.downcast::<PyInt>() {
            Err(pyo3::exceptions::PyNotImplementedError::new_err(
                "ZonedDateTime from int not implemented",
            ))
            // let i = v.extract::<i64>()?;
