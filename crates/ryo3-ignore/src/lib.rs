...
//
#[pyclass(name = "IgnoreWalk", module = "ry.ignore")]
pub struct PyIgnoreWalk {
    objects: bool,
    iter: Box<dyn Iterator<Item = ::ignore::DirEntry> + Send + Sync>,
}
//
#[pymethods]
impl PyIgnoreWalk {
...
fn walk(path: Option<PathLike>) -> PyResult<PyIgnoreWalk> {
    let path = path.unwrap_or(PathLike::Str(String::from(".")));
    println!("Walk");
    let w = ignore::Walk::new(path);
    // let iter = w.into_iter();
    Ok(PyIgnoreWalk {
        objects: false,
        iter: Box::new(w.into_iter().flatten()),
    })
}

pub fn pymod_add(m: &Bound<'_, PyModule>) -> PyResult<()> {
    // m.add_class::<PyIgnoreWalk>()?;
